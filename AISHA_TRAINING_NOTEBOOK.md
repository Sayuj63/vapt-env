...
```python
class SecurityAuditEnv(EnvClient):
...
print("✓ Environment client created")
```
...
def encode_observation(obs: SecurityAuditObservation, tokenizer) -> str:
...
def train_episode(model, tokenizer, env, device: str, episode_num: int) -> Tuple[float, List[float]]:
    ...
    while not obs.done and step < 30:
        # Encode observation
        obs_text = encode_observation(obs, tokenizer)
...
```python
print("Running baseline evaluation (random/untrained agent)...")
print("=" * 60)

baseline_rewards = []

for episode in range(5):
    print(f"\nBaseline Episode {episode + 1}/5")
    ...
    try:
        ...
        print(f"  Baseline Reward: {episode_reward:.4f}")
...
```python
print("Running trained agent evaluation...")
print("=" * 60)

trained_rewards = []

for episode in range(5):
    print(f"\nTrained Episode {episode + 1}/5")
    ...
    try:
        ...
        print(f"  Trained Reward: {episode_reward:.4f}")
...
plt.subplot(1, 2, 1)
plt.plot(training_rewards, marker='o', label='Trained Agent', linewidth=2)
baseline_avg = np.mean(baseline_rewards)
plt.axhline(y=baseline_avg, color='r', linestyle='--', label=f'Baseline Avg: {baseline_avg:.4f}')
plt.xlabel('Episode')
plt.ylabel('Reward')
plt.title('Reward Curve: Training Progress')
plt.legend()
...
plt.subplot(1, 2, 2)
categories = ['Baseline\n(Untrained)', 'Trained\nAgent']
scores = [np.mean(baseline_rewards), np.mean(trained_rewards)]
...
print("Downloading generated plots...")
